...
def main():
    ...
    report={
      ...
      'rental_possible_distribution': dict(Counter(c.get('rental_possible_status') or 'unknown' for c in queue).most_common()),
      'rental_confidence_distribution': dict(Counter(c.get('rental_possible_confidence') or 'unknown' for c in queue).most_common()),
      'rental_unlikely_items': sum(1 for c in queue if c.get('rental_possible_status') == 'unlikely'),
      'rental_unclear_items': sum(1 for c in queue if c.get('rental_possible_status') == 'unclear'),
