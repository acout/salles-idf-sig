...
function renderDetail(id){
  ...
  $('detail-content').innerHTML = `<div class="detail-inner"><h2>${esc(p.name)}</h2><div class="venue-meta">${statusPill(c.status||'new')}<span class="pill">${esc(p.department)}</span><span class="pill">${esc(p.city||'—')}</span><span class="pill fit">fit ${esc(p.fit_score)}</span></div><div class="quality-badges detail-quality">${qualityBadgesHTML(id)}</div><p class="muted">${esc(p.address||'adresse à vérifier')}</p>
    ...
    <div class="section formal-section"><h3>Scraping formel</h3><div class="detail-grid"><div><b>Type</b><br>${datasetLabel(p)}${p.aggregator_domain?` · ${esc(p.aggregator_domain)}`:''}</div><div><b>Statut scrape</b><br>${esc(p.formal_extraction_status||'non testé')} ${p.formal_scrape_http_status?`HTTP ${esc(p.formal_scrape_http_status)}`:''}</div><div><b>Complétude</b><br>${esc(p.formal_completeness_score||'—')}/4</div><div><b>Liens extraits</b><br>${esc(p.aggregator_child_links_count||'0')}</div><div><b>Location</b><br><span class="pill rental-${esc(rentalStatus(p))}">${rentalLabel(p)}</span><br><small>${esc(p.rental_possible_confidence||'')}</small></div></div>${formalMissing(p)?`<p class="missing-box"><b>À demander par email :</b> ${esc(formalMissing(p))}<br><span>${esc(formalQuestions(p))}</span></p>`:'<p class="qbadge ok inline">Infos formelles principales trouvées</p>'}${p.rental_positive_signals?`<p class="help"><b>Signaux location + :</b> ${esc(p.rental_positive_signals)}</p>`:''}${p.rental_negative_signals?`<p class="help"><b>Signaux location - :</b> ${esc(p.rental_negative_signals)}</p>`:''}${p.parent_aggregator_url?`<p class="help">Extrait depuis agrégateur : <a href="${esc(p.parent_aggregator_url)}" target="_blank">${esc(p.parent_aggregator_name||p.parent_aggregator_url)}</a></p>`:''}${p.formal_page_title?`<p class="help">Titre page : ${esc(p.formal_page_title)}</p>`:''}</div>
