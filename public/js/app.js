...
const QUALITY_LABELS = {
  contact_missing:'Contact ?', price_missing:'Prix ?', capacity_uncertain:'Capacité ?', duplicate_suspect:'Doublon ?', geocode_suspect:'Géocode ?', aggregator_listing:'Agrégateur', formal_missing:'À demander', scrape_failed:'Scrape KO'
};
...
function popupHTML(f){
  ...
  return `<div class="popup"><h3>${esc(p.name)}</h3><span class="pill dataset-${isAggregator(p)?'aggregator':esc(p._dataset||'venue')}">${datasetLabel(p)}</span> ${statusPill(c.status||'new')} <span class="pill fit">fit ${esc(p.fit_score||p.fit_beyond_score)}</span>
...
function updateMetrics(){ const tracked=Object.values(state.crm).filter(v=>v.status&&v.status!=='new').length; const quality=state.filtered.filter(f=>qualityFor(venueId(f.properties)).score>0).length; const cand=state.filtered.filter(f=>isCandidate(f.properties)).length; const aggs=state.filtered.filter(f=>isAggregator(f.properties)).length; const mapped=state.filtered.filter(coordsOf).length; $('metrics').textContent=`${state.filtered.length} affichées · ${cand} candidats · ${aggs} agrégateurs · ${mapped} cartographiées · ${quality} à vérifier · ${tracked} suivies`; }
...
function renderDetail(id){
  ...
  $('detail-content').innerHTML = `<div class="detail-inner"><h2>${esc(p.name)}</h2><div class="venue-meta">${statusPill(c.status||'new')}<span class="pill">${esc(p.department)}</span><span class="pill">${esc(p.city||'—')}</span><span class="pill fit">fit ${esc(p.fit_score)}</span></div><div class="quality-badges detail-quality">${qualityBadgesHTML(id)}</div><p class="muted">${esc(p.address||'adresse à vérifier')}</p>
    ...
    <div class="section formal-section"><h3>Scraping formel</h3><div class="detail-grid"><div><b>Type</b><br>${datasetLabel(p)}${p.aggregator_domain?` · ${esc(p.aggregator_domain)}`:''}</div><div><b>Statut scrape</b><br>${esc(p.formal_extraction_status||'non testé')} ${p.formal_scrape_http_status?`HTTP ${esc(p.formal_scrape_http_status)}`:''}</div><div><b>Complétude</b><br>${esc(p.formal_completeness_score||'—')}/4</div><div><b>Liens extraits</b><br>${esc(p.aggregator_child_links_count||'0')}</div></div>${formalMissing(p)?`<p class="missing-box"><b>À demander par email :</b> ${esc(formalMissing(p))}<br><span>${esc(formalQuestions(p))}</span></p>`:'<p class="qbadge ok inline">Infos formelles principales trouvées</p>'}${p.parent_aggregator_url?`<p class="help">Extrait depuis agrégateur : <a href="${esc(p.parent_aggregator_url)}" target="_blank">${esc(p.parent_aggregator_name||p.parent_aggregator_url)}</a></p>`:''}${p.formal_page_title?`<p class="help">Titre page : ${esc(p.formal_page_title)}</p>`:''}</div>
